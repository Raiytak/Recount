...
class ParserJsonToHtml():
    ...
    def getAddDivDiv(self, name_div, depth):
        if depth == 1:
            add_div, add_div_id = self.ReusableInputs.getAddDivDiv(name_div, 'Add a layer')
        else:
    ...
    def getRemoveDivDiv(self, name_div, depth):
        if depth == 1:
            remove_div, remove_div_id = self.ReusableInputs.getRemoveDivDiv(name_div, 'Remove a layer')
        else:
            remove_div, remove_div_id = self.ReusableInputs.getRemoveDivDiv(name_div, "-")
        return remove_div
    ...
    def getEmbodyOfDict(self, name_parent, dict_to_parse, depth):
        ...
        for key in dict_to_parse.keys():
            ...
            if type(elements_of_dict) == list:
                depth +=1
                html_final_group = self.getFinalGroupOfHDivOfListAndDepth(key, elements_of_dict, depth)
