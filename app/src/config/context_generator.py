...
class ContextGenerator:
    def __init__(self):
        ...
        self.path_keys_folder = self.configPath.getKeysFolderPath()

    def generate_private_key(self, filename: str, passphrase: str):
        private_key = rsa.generate_private_key(
            public_exponent=65537, key_size=2048, backend=default_backend()
        )
        utf8_pass = passphrase.encode("utf-8")
        algorithm = serialization.BestAvailableEncryption(utf8_pass)
        with open(self.path_keys_folder / filename, "wb") as keyfile:
            keyfile.write(
    ...
    def generate_public_key(self, private_key, filename, **kwargs):
        subject = x509.Name(
            [
        ...
        with open(self.path_keys_folder / filename, "wb") as certfile:
            certfile.write(public_key.public_bytes(serialization.Encoding.PEM))
        return public_key

    def generate_csr(self, private_key, filename, **kwargs):
        subject = x509.Name(
            [
                x509.NameAttribute(NameOID.COUNTRY_NAME, kwargs["country"]),
                x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, kwargs["state"]),
        ...
        with open(self.path_keys_folder / filename, "wb") as csrfile:
            csrfile.write(csr.public_bytes(serialization.Encoding.PEM))

        return csr

    def sign_csr(self, csr, ca_public_key, ca_private_key, filename):
        valid_from = datetime.utcnow()
    ...
    def generate_keys(self):
        private_key = self.generate_private_key("ca-private-key.pem", "secret_password")
        self.generate_public_key(
            ...
            locality="Paris",
            org="Recount",
            hostname="recount.org",
        )

        server_private_key = self.generate_private_key(
        ...
        csr_file = open(self.path_keys_folder / "server-csr.pem", "rb")
        csr = x509.load_pem_x509_csr(csr_file.read(), default_backend())

        ca_public_key_file = open(self.path_keys_folder / "ca-public-key.pem", "rb")
        ca_public_key = x509.load_pem_x509_certificate(
            ca_public_key_file.read(), default_backend()
        )

        ca_private_key_file = open(self.path_keys_folder / "ca-private-key.pem", "rb")
        ca_private_key = serialization.load_pem_private_key(
            ca_private_key_file.read(), getpass().encode("utf-8"), default_backend(),
        )

        self.sign_csr(csr, ca_public_key, ca_private_key, "server-public-key.pem")
...
if __name__ == "__main__":
    ...
    contextGenerator.generate_keys()
