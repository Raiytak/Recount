...
class DataframeToListOfDicts:
    def convertDataframeToListOfDicts(self, dataframe, type_graph):
        if type_graph == "scatter-graph-all":
            list_dict_expenses = self.convertDataframeToExpensesByColumn(
                dataframe, "category"
            )
            return list_dict_expenses

        if type_graph == "pie-chart-category":
            list_dict_expenses = self.convertDataframeToSumExpensesByColumn(
                dataframe, "category"
            )
            return list_dict_expenses
        ...
        if type_graph == "mean-bar":
            ...
            list_dict_expenses = self.convertListDataframeByWeekToExpensesByColumn(
                dataframe, "category"
            )
            return list_dict_expenses
    ...
    def convertDataframeToSumExpensesByColumn(self, dataframe, column_name):
        ...
        unique_theme.sort()
        for i in unique_theme:
            list_dict_expenses["values"].append(
                np.sum(data[data[column_name] == i]["amount"])
            )
            list_dict_expenses["names"].append(i)
            list_dict_expenses["labels"].append(i)
        return [list_dict_expenses]

    def convertListDataframeByWeekToExpensesByColumn(
        self, list_dataframe_by_week, column_name
    ):
        dict_returned = {}
        for dataframe in list_dataframe_by_week:
            data = dataframe
            unique_theme = [
                val for val in data[column_name].unique() if val is not None
            ]
            dict_expenses = {
                i: dict(
    ...
    def convertDataframeOfWeeksToListOfDicts(
        self, dataframe_by_week, type_graph
    ):  # dataframe of format [ [datetime.datetime(xxx), dataframe], ... ]
        if type_graph == "mean-bar":
    ...
    def convertDataframeToFoodByWeek(self, dataframe_by_week):
        ...
        for date, data in dataframe_by_week:
            if data.empty:
                ...
                dict_expenses_week = {
                    i: dict(
                        x=[date], y=[np.sum(data[data["theme"] == i]["amount"])], name=i
                    )
                    for i in unique_theme
                }
            dict_prep_expenses = updateDictWithLists(
                dict_prep_expenses, dict_expenses_week
            )
